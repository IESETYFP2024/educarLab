...
const ComunidadForm = ({ talleres }) => {
  ...
  const fetchCuposDisponibles = async (tallerId) => {
    try {
      const response = await axios.get(`${IP}/capacidad-taller-comunidad/${tallerId}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const capacidadResponse = await axios.get(`${IP}/capacidad-taller-comunidad/${tallerActual.id}`);
      ...
      if (capacidadResponse.data.turnosDisponibles > 0) {
        ...
        await axios.post(`${IP}/post/comunidad`, {
          ...formData, 
