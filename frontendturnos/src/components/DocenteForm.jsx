...
const DocenteForm = ({ talleres }) => {
  ...
  const fetchCuposDisponibles = async (tallerId) => {
    try {
      const response = await axios.get(`${IP}/capacidad-taller-docente/${tallerId}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const capacidadResponse = await axios.get(`${IP}/capacidad-taller-docente/${tallerActual.id}`);
      ...
      if (capacidadResponse.data.turnosDisponibles > 0) {
        ...
        await axios.post(`${IP}/post/docente`, {
          ...formData, 
