...
//     fetch('http://localhost:3002/api/alumnos')
//         .then(response => {
//             if (!response.ok) {
...
//             const response = await fetch('http://localhost:3002/api/docentes');
//             if (!response.ok) {
...
function loadTalleres() {
    ...
    fetch('http://localhost:3002/api/alumnos')
        ...
        .then(data => {
            ...
            if (data.length === 0) {
            document.querySelector('.prev-inicio').style.display = 'none';
            document.querySelector('.next-inicio').style.display = 'none';
        }
