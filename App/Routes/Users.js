...
const transporter = nodemailer.createTransport({
  service: 'gmail',
  auth: {
    user: 'educarlab@elechaco.edu.ar',
...
users.post('/sendResetPasswordEmail', async function(req, res) {
  ...
  try {
    ...
    if (rows.length > 0) {
      ...
      const mailOptions = {
        from: 'educarlab@elechaco.edu.ar',
        to: email,
        subject: 'Restablecimiento de contraseña',
        text: `Haga clic en el siguiente enlace para restablecer su contraseña: https://educarlab.gar.com.ar/resetPassword.html?token=${token}`
      };
